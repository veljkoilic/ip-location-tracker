<template>
  <div class="absolute -bottom-64 lg:-bottom-20 flex flex-col lg:flex-row bg-white gap-y-4 gap-x-12 px-8 py-8 pb-12 rounded-md">
<!--    Flex Intem #1-->
    <div class="flex flex-col">
      <h3 class="text-xs mb-2 uppercase">IP Adress</h3>
      <span class="text-2xl">{{ipInfo.address}}</span>
    </div>

    <!--    Flex Intem #2-->
    <div class="flex flex-col">
      <h3 class="text-xs mb-2 uppercase">Location</h3>
      <span class="text-2xl">{{ipInfo.state}}, {{ipInfo.region}}</span>
    </div>

    <!--    Flex Intem #3-->
    <div class="flex flex-col">
      <h3 class="text-xs mb-2 uppercase">Timezone</h3>
      <span class="text-2xl">UTC {{ipInfo.timezone}}</span>
    </div>

    <!--    Flex Intem #4-->
    <div class="flex flex-col">
      <h3 class="text-xs mb-2 uppercase">ISP</h3>
      <span class="text-2xl">{{ipInfo.isp}}</span>
    </div>
  </div>
</template>
<script>
export default {
  props:[
      'ipInfo'
  ]
}
</script>
